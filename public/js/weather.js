  ...
  async (position) => {
    ...
    try {
      ...
      if (rainChance < 30 && temp > 8) {
        document.getElementById('explore_time').textContent =
          "☀️ It's a great weather to explore !!";
      } else if (temp < 0) {
        document.getElementById('explore_time').textContent =
          'Beware of snow, stay safe';
      } else {
        document.getElementById('explore_time').textContent =
          '🌧️ It’s always a good time to explore—just dress for the weather and stay safe!';
      }
    } catch (err) {
